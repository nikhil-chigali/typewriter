...
function saveConfig() {
  cfgStore.writeConfig({ recordsDir, activePlan: cfg.activePlan, muted: cfg.muted, scale });
}
...
ipcMain.handle('tw:init', async () => {
  recordsDir = await cfgStore.resolveRecordsDir(cfg, win);
...
ipcMain.handle('tw:import-file', async (_e, filePath) => {
  try {
    ...
    const dest = lib.freePath(recordsDir, base, ext);
    fs.copyFileSync(filePath, dest);

    const plan = lib.loadPlan(dest);
...
ipcMain.handle('tw:import-text', async (_e, text) => {
  try {
    ...
    const dest = lib.freePath(recordsDir, md.slugify(parsed.title), '.md');
...
ipcMain.handle('tw:toggle', async (_e, { index, done } = {}) => {
  try {
    ...
    if (!planPath || !lib.isInside(recordsDir, planPath)) return fail('no active plan');
...
ipcMain.handle('tw:complete', async (_e, note) => {
  try {
    const planPath = cfg.activePlan;
    if (planPath && lib.isInside(recordsDir, planPath) && typeof note === 'string' && note.trim()) {
      const text = fs.readFileSync(planPath, 'utf8');
...
ipcMain.handle('tw:abort', async (_e, { action, doneCount, total } = {}) => {
  const planPath = cfg.activePlan;
  try {
    if (planPath && lib.isInside(recordsDir, planPath)) {
      if (action === 'log') {
        const text = fs.readFileSync(planPath, 'utf8');
        ...
        fs.rmSync(lib.sidecarPath(planPath), { force: true });
      }
    }
  } catch (err) {
